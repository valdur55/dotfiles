...
class Py3status:
            ...
            if status:
                if sender_player:
                    ...
                    if sender_player["_dbus_player"].PlaybackStatus == status:
                        ...
                        if "chromium" in sender_player_id and status == "Stopped":
                            return self._remove_player(sender_player_id, sender)
    ...
    def mpris(self):
        ...
        if self._player:
            self._init_data()
            if not self._data.get("exception") and current_player_id == self._player_details.get(
                "_id"
            ):
...
if __name__ == "__main__":
    ...
    module_test(Py3status, config={"format": "{time}"})
